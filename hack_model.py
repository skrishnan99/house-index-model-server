...
if __name__ == "__main__":
    ...
    ram_checkpoint = "./Tag2Text/pretrained/ram_swin_large_14m.pth"  # change the path of the model
    ...
    ram_inference_start = time.perf_counter()
    ...
    print("ram inference time", time.perf_counter() - ram_inference_start)

    # run grounding dino model
    model = load_model(config_file, grounded_checkpoint, device=device)
    dino_inference_start = time.perf_counter()
    boxes_filt, scores, pred_phrases = get_grounding_output(
        model, image, tags, box_threshold, text_threshold, device=device
    )
    print("dino inference time", time.perf_counter() - dino_inference_start)
    ...
    print("sam inference time", time.perf_counter() - sam_start)
